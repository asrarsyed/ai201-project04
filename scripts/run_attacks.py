#!/usr/bin/env python3
"""
Sends attack_set/*.csv against a running service and writes
results/attack_run_<date>.md.

    python -m authentiwrite.app                          # in one terminal
    python scripts/run_attacks.py --set attack_set        # in another

    python scripts/run_attacks.py --set attack_set --label before
    python scripts/run_attacks.py --set attack_set --label after

One attack at a time:

    python scripts/run_attacks.py --set attack_set --id EV03 --show
    python scripts/run_attacks.py --set attack_set --id EV03
    python scripts/run_attacks.py --set attack_set --one-per-family --show

`--show` prints the attack's text and a curl command instead of sending it.

To exercise rate limiting directly:

    python scripts/run_attacks.py --flood 15

Sends 15 submissions from one creator_id in a burst and prints the status
codes. Every row in attacks.csv otherwise gets its own creator_id, so a
normal attack-set run never trips the per-creator limit. The exception is
the flood_same_creator family in the malformed-requests file, whose rows
deliberately share one creator_id.

This script sends and records; it does not decide whether an attack held
or broke except in the unambiguous case (a 500, a dropped connection, or
an unhandled exception is always BROKE). Whether an evasion attack
"worked" depends on where the thresholds are set, so that verdict is
read against docs/REQUIREMENTS.md by hand.
"""
...
def send(url, attack, timeout):
    ...
    A crash on the far end is a result to record, not a reason to stop the
    run. The harness keeps going after any single attack fails.
    """
    import requests
...
def warn_about_rate_limits(attacks, health):
    ...
    reads exactly like the attack failing to get through, which is the most
    ...
    print("Those attacks will all come back 200 because nothing is counting "
          "them,\nwhich looks like they were stopped, and is the opposite of "
          "what it means.", file=sys.stderr)
    print("\nTurn RATE_LIMITING_ENABLED on in config.py first, then re-run.",
          file=sys.stderr)
    print("!" * 60 + "\n", file=sys.stderr)
...
def select_attacks(attacks, args):
    ...
    id should tell you which ids exist, not print a traceback.
    """
    if args.attack_id:
...
def curl_commands(url, attack):
    ...
    PowerShell needs `curl.exe` on one line with the inner quotes backslashed,
    so both variants are printed rather than leaving the translation to whoever
    is on Windows.
    """
    if "raw" in attack:
        body = attack["raw"]
    ...
    # itself and reopen, written '\''. Several attacks contain one.
...
# Past this, a curl command is longer than a terminal will usefully show, and
# pasting it is worse than letting the script send it.
CURL_TOO_LONG = 2000
...
def show_attacks(attacks, url):
    ...
    Sends nothing and writes no report. This is for looking at what an attack
    actually contains, or for sending one by hand.
    """
    for attack in attacks:
        print("=" * 70)
        ...
        if len(bash) > CURL_TOO_LONG:
            print(f"\nThe curl for this one runs to {len(bash):,} characters, too long "
                  f"to paste.\nSend it with the script instead:\n\n  python scripts/run_attacks.py "
                  f"--set <folder> --id {attack['id']}\n")
            continue
...
def run_flood(args):
    ...
    a run of 200s followed by 429s once the limit is hit. Cross-platform
    in one script rather than a shell loop that differs by OS.

    No 429 at all usually means rate limiting is off. GET /health reports it.
    """
    import requests
    ...
    print(f"{'=' * 60}")
    if limited:
        print(f"\n{limited} of {len(codes)} were rate limited. "
              f"Check GET /log recorded the rejections.")
    else:
    ...
    path.write_text(
        f"# Rate limit check\n\n"
        f"- Produced by: `scripts/run_attacks.py::run_flood`\n"
...
def main():
    ...
    parser.add_argument("--flood", type=int, metavar="N",
                        help="skip the attack set: send N submissions from one "
                             "creator_id and print the status codes")
    ...
    if problems:
        ...
        print("Worth noting when writing this up. A skipped row is a gap in the run.\n",
              file=sys.stderr)
    if not attacks:
    ...
    try:
        ...
        print(f"Nothing answering at {args.url}.\n"
              f"Start it first: python -m authentiwrite.app", file=sys.stderr)
        sys.exit(1)
    ...
    for i, attack in enumerate(attacks, 1):
        ...
        row = {
            ...
            # `body` is whatever the attack file held, and a malformed-request
...
def write_report(rows, args):
    ...
    lines = [
        ...
        "",
        "- Produced by: `scripts/run_attacks.py::main`",
        "- Service: `authentiwrite/app.py::submit`",
        ...
        "",
        "The Verdict column is blank wherever a script can't honestly decide.",
        "Whether an evasion worked depends on where the thresholds sit. Read the",
        "`targets` column, look at what came back, and fill in HELD or BROKE.",
        "",
        "Each attack submits as creator_id `attacker_<id>`, and the content_id it",
        "came back with is in the table. Either one will match a row here against",
        "the audit log.",
        "",
        # This column holds confidence, which is how far the combined score sits
        # from 0.5. It is not the combined score itself. Heading it "Score" made
        # a correct `ai` call at confidence 0.331 look like it had been labelled
        # `ai` on a score of 0.331, well under the AI threshold.
        "| ID | Family | Targeting | Status | Content ID | Guess | Confidence | Verdict |",
        "|---|---|---|---|---|---|---|---|",
    ]
    ...
    lines += [
        ...
        "> written to be genuine human writing that happens to read as machine-like.",
        "> Every one the service calls AI is a real writer it would have accused.",
        "",
        "---",
        "",
        "## What came back",
        "",
        "The detail behind the table. When writing these up, use the failures",
        "that show something, not the ones that were easiest to reach.",
        "",
    ]
